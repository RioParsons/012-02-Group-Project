...
app.get('/discover', (req, res) => {
  ...
  const topRestaurantsQuery = `SELECT r.restaurant_id, r.name, r.image_url, AVG(rt.rating_number) AS average_rating FROM restaurants r JOIN ratings rt ON r.restaurant_id = rt.restaurant_id GROUP BY r.restaurant_id, r.name, r.image_url ORDER BY COUNT(rt.rating_id) DESC LIMIT 4;`;
  const highestRatedQuery = `SELECT r.restaurant_id, r.name, r.image_url, AVG(rt.rating_number) AS average_rating FROM restaurants r JOIN ratings rt ON r.restaurant_id = rt.restaurant_id GROUP BY r.restaurant_id, r.restaurant_id ORDER BY average_rating DESC LIMIT 4;`;
  const weeksEventsQuery = `SELECT r.restaurant_id, r.name, r.image_url, e.event_title, AVG(rt.rating_number) AS avg_rating FROM restaurants AS r JOIN events AS e ON r.name = e.restaurant LEFT JOIN ratings AS rt ON r.restaurant_id = rt.restaurant_id WHERE e.day LIKE '%${currentDay}%' GROUP BY r.restaurant_id, r.name, r.image_url, e.event_title ORDER BY RANDOM() LIMIT 4;`;
  const dailyDealsQuery = `SELECT r.restaurant_id, r.name, r.image_url, d.deal_title, AVG(rt.rating_number) AS avg_rating FROM restaurants AS r JOIN deals AS d ON r.name = d.restaurant LEFT JOIN ratings AS rt ON r.restaurant_id = rt.restaurant_id WHERE d.day LIKE '%${currentDay}%' GROUP BY r.restaurant_id, r.name, r.image_url, d.deal_title ORDER BY RANDOM() LIMIT 4;`;
